...
def test_variants_reuse_the_base_weapons_face():
    assert render._weapon_art_slug("keen_iron_sword") == "iron_sword"
    assert render._weapon_art_slug("warded_iron_sword") == "iron_sword"
    assert render._weapon_art_slug("iron_sword") == "iron_sword"
    # not a weapon → no weapon art
    assert render._weapon_art_slug("padded_jerkin") == ""


def test_every_line_slug_resolves_a_data_url():
    for slug in _design_slugs():
        assert render._weapon_art_url(slug, "icons"), slug
        assert render._weapon_art_url(slug, "large"), slug


def test_the_shop_row_icon_is_the_weapons_own():
    html = render._opt_gear_icon("buy_iron_sword")
    assert "gicon gw" in html
    assert render._weapon_art_url("iron_sword", "icons") in html
    # armor keeps the shared 16×16 glyph
    plate = render._opt_gear_icon("buy_padded_jerkin")
    assert plate and "gw" not in plate
...
def test_forge_card_tip_carries_the_portrait():
    p = create_character(fresh("Tipsy"))
    p["location"] = "forge"
    s = core.current_scene(p)
    html = render.render_scene(s)
    # the fresh rack sells rung 1.0 — its card carries the portrait
    lurl = render._weapon_art_url("scrap_dagger", "large")
    assert lurl in html                       # the portrait rides the tip
    assert "data-tiph" in html
    # the param line is escaped inside the attribute — check the ink
    assert f"color:{render.GOLD}" in render._weapon_card_tip(
        "buy_scrap_dagger")
...
def test_arcanum_is_a_card_wall_with_one_line_and_a_legend():
    ...
    assert render._weapon_art_url("kindling_rod", "icons") in html
