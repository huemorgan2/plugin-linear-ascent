# 013 · Scenario 1 — combat feel: chips, named gear, explained blows
...
1. With a floor-1 character wearing any armor/shield, hunt the wilds.
2. **Expect (opener):** below the creature's prose, a line with your own
   numbers — `You — ATK <n> with your <weapon>, DEF <n> behind your
   <armor and shield>.` (bare characters read `on reflex alone`).
3. Attack until the fight ends. **Expect every round:**
   - your blow names the weapon (`Your Rusted Shiv takes it for 4.`);
     a blow ≥ ⅓ of the enemy's HP pool reads `bites deep — <n> damage
     the <name> won't shrug off`;
   - the counter-blow NEVER reads a bare `It answers for 0`; when armor
     ate most of it: `your <armor> soak almost all of it: only −1 HP
     gets through`; when part: `blunted <n> of it`; when bare: `nothing
     between you and its teeth`.
4. Watch the HP meter: **every** enemy blow moves it by ≥ 1 (except a
