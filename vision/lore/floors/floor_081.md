# Floor 81 — The First Outwork (Made)

> *"Below this, every floor was a place the King stole. From here up, it's all a thing the King built — and he built it out of us." — Sapper-captain Dara Sappersrest*

**The Made:** built, not stolen — the tower's own manufacture · **Gate-town:** Sappersrest (a captive labor-camp) · **Warden:** Rivetgrim

## The land
The tower's first siege-outwork — the beginning of the Made, where the stolen
world ends and Vharuk's own construction begins: a brutal earth-and-iron
rampart of riveted plate, gun-less embrasures, and sapping-trenches, thrown up
as the outer defense of the King's works above. There is no nature here at
all: no soil, no green, only slag-fill, iron, and rivet. Crows and rats infest
it as they infest any fortress, but nothing *grows*. It was raised by slave-
sappers dragged up from the freed floors below, and their bones are in the
fill.

## "Flora" — the Making's growth
- **Canopy:** none — riveted plate and trench-timber.
- **Ground growth:** rust-weed in the sap-trenches, slag-lichen on the plate,
  and iron-bramble (welded wire grown like thorn) on the ramparts.
- **How the Making twists it:** the iron-bramble is barbed wire given a
  cruel half-life that grasps; the rust-weed corrodes a climber's gear; the
  slag-lichen sparks the trench-gas.

## Three places
- **The cave — the Sap-Head.** The forward head of the sapping-tunnel driven
  under the outwork by the slave-sappers. *Seed: the sappers cached demolition-
  lore and a charge here — enough to bring the whole outwork down from below.*
- **The peak — the Outwork Cavalier.** The high raised rampart overlooking the
  Made's first floors; the siege-works read from it. *Seed: from it the
  tower's outer defenses can be mapped for the whole rising.*
- **The gate in the mountain — the Outwork Gate.** The stair-lift is set
  behind the outwork's riveted sally-port. *Seed: Rivetgrim holds the port.*

## The people — who you are freeing
Captive sappers — folk dragged back into bondage from the freed floors below
(and captured climbers) and set to dig the tower's siege-works, worked to
death and rivet by rivet built into the walls. This is the Made's cruelty laid
bare: it is built *of the liberated,* re-enslaved. They love a tunnel shored, a
shift survived, and the rumor of the rising coming up from below. They fear the
overseers' welds and being built into the fill alive.
**The keeper:** Dara Sappersrest, sapper-captain — keeps the demolition-lore
and the tally of the dead in the walls, and waits for the charge to be lit.

## The six  *(the Made's making — Pressed soldiers and Wrongmade constructs)*
1. **Outwork crow** (N) — *was:* a fortress-scavenging crow. *now:* iron-
   daubed, mobbing the ramparts. *Cured, it flaps off the plate.*
2. **Furnace rat** (N) — *was:* a slag-fill rat. *now:* soot-bold, massing in
   the trenches. *Cured, it bolts into the fill.*
3. **Rivet-tick** (N) — *was:* a plate-parasite. *now:* iron-cased, dropping
   to burrow at a seam. *Cured, it drops off into a rivet-hole.*
4. **Outwork imp** (P) — *was:* a captured soul forged into a small tower-
   soldier, collar-bound to the works. *now:* a scuttling rivet-fisted
   drudge that fights and re-fights for the King. *Pressed — Break-collar
   (learned at floor 20) frees the soul; without it, only a real death.*
5. **Gate hellknight** (P) — *was:* a stronger captured soul welded into
   heavy plate, collared to guard. *now:* a faceless riveted knight holding
   the sally-port. *Pressed — the collar can be broken; freed knights are the
   rising's heaviest infantry.*
6. **Wall-flesh** (W) — *was:* sappers built into the fill alive, aether
   pooled in the mortared bone. *now:* a groaning section of rampart that
   reaches out of the plate to drag the living in. *Broken, the wall goes
   still and drains, and the dead in it are freed to rest.*

## Three finds  *(quest seeds)*
- **Unrivet — spell.** Bursts rivets, bolts, and plate-seams at a word —
  opens the Made's armor and its constructs; the first un-making art. *Seed:
  learn it to breach the outwork and every plated horror above.*
- **Rivet-steel — material.** The tower's own outwork-plate, turned against it
  — the heaviest salvaged armor the rising can field. *Seed: strip the
  ramparts.*
- **Sapper's tot — potion.** A grim labor-camp brew that steadies the hands
  and nerves for demolition-work under fire.

## The Warden — Rivetgrim
A mass of wall-flesh welded into a walking section of riveted rampart — the
outwork's own wall risen up, studded with the bones of the sappers built into
it. A pure construct of the Making, it fights by dragging climbers into its
plate and sealing them there. Its end is an *eviction:* Unrivet it and break
it, and the borrowed aether is driven out, the wall falls to dead plate, and
the sappers in it are freed to rest. The Outwork Gate opens.

## When it falls
Sappersrest lights the cached charge and the first outwork comes down from
below; the captive sappers walk out of a wall that was eating them. This is
where the climb learns the Made's truth — that the tower is built of the
re-enslaved — and where **Break-collar** from floor 20 becomes the war's whole
purpose again. The freed sappers climb as the host's breachers and demolition-
crews, carrying **Unrivet** and rivet-steel into the King's works above.
